...
- Sleep/wake disables the core's event tap (`ERROR: quartz event tap was disabled by user input`) and Deskflow never re-arms it: hotkeys silently dead. The wrapper restarts the core on that log line. An HDMI replug is harmless; `g9` looks the monitor up by model each call anyway.
