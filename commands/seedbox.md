...
- **Download client**: Sonarr+Radarr → qBittorrent via reverse proxy (`<ssh_host>:443`, SSL, urlbase `/qbittorrent`). Completed downloads seed until qBittorrent's global limit (ratio 2 or 14 days → Stop), then Sonarr/Radarr's **Remove Completed** deletes the torrent and its download copy; the library hardlink stays. Without a global limit nothing ever counts as done and downloads pile up forever (was the case until 2026-09-25). Non-*arr categories (games, audiobooks) are only stopped, never deleted.
- **Deleting media**: delete in Jellyfin (or Sonarr/Radarr). **Unmonitor Deleted Episodes/Movies** is on, so the next disk rescan (Refresh Series, 12 h) unmonitors it and it's never grabbed again; with the download copy gone after seeding, the delete frees the space and is final (no recycle bin). History reason `MissingFromDisk` = deleted outside Sonarr/Radarr.
...
Serves the Calibre library at `~/media/Books` (EPUB-only clone of Tora's 340 DRM-free Kindle books, built from `~/Desktop/projects/kindle-pw1-rescue/calibre-library-epub/` on the Mac — see that project's `HANDOFF.md`). Tora's PW1 pulls books over Wi-Fi: KOReader → OPDS catalog "Tora's Library" → `https://<ssh_host>/calibre-web/opds/` with her login; downloads land in `/mnt/us/documents/Library`.
...
- **Tora's user manual** (artifact, shareable): https://claude.ai/code/artifact/fbf5c58c-13cd-40c0-96ca-9f4d101e8e04 — source `~/Desktop/projects/kindle-pw1-rescue/TORAS-KINDLE-GUIDE.html`. Keep it in sync when URLs/flows change.
