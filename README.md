...
- **ET-8550 print queue + "4x2 Glossy" preset** — the queue itself isn't automated: add the printer in *System Settings → Printers & Scanners* (it appears via AirPrint/Bonjour; keep the default name so CUPS calls it `EPSON_ET_8550_Series`). Once it exists, the chezmoi-managed `~/.cups/lpoptions` supplies the CLI instance `EPSON_ET_8550_Series/4x2-glossy` (`lp -d EPSON_ET_8550_Series/4x2-glossy file.pdf`; used by `make-proxies --print`). The **GUI** print dialog can't see CUPS instances, so save the same thing once as a macOS preset: open a proxy sheet in Preview → Print → Paper Size **A4**, Scale **100%** (never "scale to fit"), *Media & Quality*: Feed from **Rear Paper Feed**, Media Type **Photo Paper Glossy**, Quality **Best**; *Presets → Save Current Settings as Preset…* named `4x2 Glossy`. That lands in `~/Library/Preferences/com.apple.print.custompresets.forprinter.EPSON_ET_8550_Series.plist`, a binary plist that isn't worth managing. Source of truth for the settings: `docs/printer-presets/` in [j0nas/mtg-proxy](https://github.com/j0nas/mtg-proxy). Note the option names in `lpoptions` are the AirPrint/IPP driver's; a queue created with Epson's own driver uses different ones.
