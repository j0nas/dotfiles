...
- **Config validation hook** (`.githooks/pre-commit`, installed by `run_onchange_install-git-hooks.sh.tmpl` setting `core.hooksPath` to the tracked `.githooks/`). On every commit it: renders every `*.tmpl` via `chezmoi execute-template`, checks every JSON is well-formed (`jq`), and validates files with a published schema against the **pinned** vendored copies in `.githooks/schemas/` (`check-jsonschema` for JSON, `taplo` for TOML — both mise tools). `opencode.json` is syntax-only on purpose (its schema's model enum rejects local models); VS Code settings are syntax-only because no static schema exists. `.githooks/` and `.taplo.toml` are `.chezmoiignore`d (repo-only). `git commit --no-verify` bypasses. To refresh a schema, re-download into `.githooks/schemas/` (see its README).
