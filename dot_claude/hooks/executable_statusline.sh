...
# time until an epoch (ceil days so "3d23h" reads as 4d, not 3d)
left() { local r=$(( ${1%.*} - $(date +%s) )); [ "$r" -lt 0 ] && r=0
  if   [ "$r" -ge 86400 ]; then printf '%dd' $(( (r + 86399) / 86400 ))
  elif [ "$r" -ge 3600 ];  then printf '%dh%02dm' $((r / 3600)) $(((r % 3600) / 60))
